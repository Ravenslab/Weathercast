...
const Map = dynamic(() => import('@/components/Map'), {
  ssr: false,
  loading: () => <div style={{ height: '500px', width: '100%' }}>Loading map...</div>,
});
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '20px' }}>
      ...
      {weather && (
        ...
        >
          ...
          <p>
            <strong>Humidity:</strong> {weather.main.humidity}% | <strong>Pressure:</strong>{' '}
            {weather.main.pressure} hPa
          </p>
          <p>
            <strong>Wind:</strong> {weather.wind.speed} m/s at {weather.wind.deg}°{' '}
            {weather.wind.gust ? `(gust ${weather.wind.gust} m/s)` : ''}
          </p>
          <p>
            <strong>Clouds:</strong> {weather.clouds.all}% | <strong>Visibility:</strong>{' '}
            {weather.visibility} m
          </p>
          <p>
            <strong>Sunrise:</strong> {formatTime(weather.sys.sunrise)} | <strong>Sunset:</strong>{' '}
